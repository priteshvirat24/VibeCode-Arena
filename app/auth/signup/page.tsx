      ...
      >
        <Card className="bg-black/40 backdrop-blur-3xl border border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.8)] relative overflow-hidden group rounded-2xl">
          ...
          <CardHeader className="space-y-4 text-center relative z-10 pt-8">
            ...
            <div>
              <CardTitle className="text-xl font-medium tracking-tight text-white/90">Create an account</CardTitle>
              <CardDescription className="text-white/40 mt-1">Join the Arena and start building.</CardDescription>
            </div>
          </CardHeader>
          <CardContent className="space-y-4 relative z-10">
            <form onSubmit={handleEmailSignup} className="space-y-4 relative z-10">
              <div className="space-y-2.5">
                <label className="text-[13px] font-medium text-white/70" htmlFor="username">
                  Username
                </label>
                <Input id="username" type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="vibecoder_99" className="h-10 bg-white/[0.03] border-white/10 focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/30 transition-all text-white placeholder:text-white/20 rounded-xl" required />
              </div>
              <div className="space-y-2.5">
            ...
            <Button onClick={handleOAuthSignup} variant="outline" type="button" className="w-full h-10 bg-white/[0.02] border-white/10 hover:bg-white/[0.05] hover:text-white text-white/80 transition-colors rounded-xl font-medium">
          ...
          <CardFooter className="flex justify-center relative z-10 border-t border-white/5 pt-6 mt-4 bg-white/[0.01]">
            <div className="text-[13px] text-white/50">
              ...
              <Link href="/auth/login" className="text-white hover:text-indigo-300 font-medium transition-colors">
                Sign in
              </Link>
            </div>
          </CardFooter>
